#import file and libraries upload
design<-function(dataset){
library(readxl)
library(choiceDes)
...
set.seed(1)
design<-dcm.design.cand(cand=cand,sets=as.numeric(dat[,length(dat)-2][1]),alts=as.numeric(dat[,length(dat)-1][1]),nb=as.numeric(dat[,length(dat)][1]))
...
file<-write.csv(design,file=fileName)
z<-list(design, file)
return(z)
